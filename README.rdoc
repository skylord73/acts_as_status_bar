=ActsAsStatusBar
ActsAsStatusBar add status bar functionality to your ActiveRecord Models
I tried to keep it very simple, but powerfull enought to be very usefull.
...
==Usage
Lets immagine you'll have a Home model, with controller and view...
...
(the helper create the bar id, but form_for copy the object so is not possibile update by reference...)
...
When you status_bar_init in your controller, status_bar_id is nil so a new one is created,
stored in status_bar_id, and passed to your view with your object.

You need to pass it back to your controller create actions using an hidden field.

When you call status_bar_init from your model the status_bar_id is valorized, so your status_bar
now points to the same status_bar you passed to your helpers in the view, that create the js progress bar.
...
===Basic
*	status_bar_init
	 Initialize the bar using the id in status_bar_id, ore create a new one if not defined.
* status_bar.inc(value=1)
	 Incrent counter by 1 or by value
*	status_bar.dec(value=1)
	 Decrement counter by 1 or by value
* status_bar.max=(value)
	 Set the max value, that when reached stops the bar
...
* starus_bar.delete
   ...
   Do not forget or you will be submerged of useless inactive status bars...
...
* status_bar.progress=(value)
 ...
 You can customize the first, and the last fileds, while the second (percentage) is used for the visual bar and to
 stop the bar when 100 is reached.
 You can see then code to get the usable fields.
* status_bar.add_field(:field)
   add a new field to the status bar, you can use to store data, or to customize the output
   new methods are created to access your new field:
 * inc_field
 * dec_field
 * field=(value)
 * field
